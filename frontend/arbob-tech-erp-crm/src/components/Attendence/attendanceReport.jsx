...
// export default AttendanceReport;import React, { useState, useEffect } from 'react';
...
function AttendanceReport() {
  const [selectedEmployee, setSelectedEmployee] = useState('All Employees');
  const [year, setYear] = useState(new Date().getFullYear().toString());
  const [month, setMonth] = useState(new Date().toLocaleString('default', { month: 'long' })); // Current month name
  const [reportData, setReportData] = useState([]);
  const [filteredReportData, setFilteredReportData] = useState([]); // For filtered data based on employee
  const navigate = useNavigate();

 // Memoize the employees array so it doesn't change on every render
 const employees = useMemo(() => [
  'John Doe', 'Jane Smith', 'Michael Johnson', 'Emily Brown', 'David Wilson'
], []); // Empty dependency array, so it is memoized and won't be recreated on each render
  ...
  useEffect(() => {
    // Generate random attendance data with "Present", "Absent", and "Leave"
    ...
    setReportData(dummyReportData);
    setFilteredReportData(dummyReportData); // Initially show all employees
  }, [employees]); // <-- Add employees as a dependency here
  

  const handleGenerateReport = () => {
    console.log('Generating report for:', selectedEmployee, year, month);

    // Filter the report data based on the selected employee
    if (selectedEmployee === 'All Employees') {
      setFilteredReportData(reportData); // Show all employees
    } else {
      const filteredData = reportData.filter(
        (employee) => employee.name === selectedEmployee
      );
      setFilteredReportData(filteredData); // Show only the selected employee
    }
  };

  const calculateTotals = (attendance) => {
    const totalPresent = attendance.filter((status) => status === 'Present').length;
    const totalAbsent = attendance.filter((status) => status === 'Absent').length;
    const totalLeave = attendance.filter((status) => status === 'Leave').length;
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Total Present</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Total Absent</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Total Leave</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredReportData.map((employee, index) => {
                    ...
                    {employee.attendance.map((status, dayIndex) => (
                      <td key={dayIndex} className="px-2 py-4 whitespace-nowrap text-sm text-center">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          ...
                          'bg-yellow-100 text-yellow-800' // For "Leave"
                        }`}>
                          {status.charAt(0)}
                        </span>
                      </td>
                    ))}
                    <td className="px-6 py-4 text-center text-sm text-green-600 font-semibold">{totals.totalPresent}</td>
                    <td className="px-6 py-4 text-center text-sm text-red-600 font-semibold">{totals.totalAbsent}</td>
                    <td className="px-6 py-4 text-center text-sm text-yellow-600 font-semibold">{totals.totalLeave}</td>
                  </tr>
                );
              })}
            </tbody>
