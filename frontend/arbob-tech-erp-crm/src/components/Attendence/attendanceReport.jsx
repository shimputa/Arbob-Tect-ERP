...
function AttendanceReport() {
  ...
  return (
    <div className={`container mx-auto p-4 sm:p-6 ${isDarkMode ? 'bg-dark-primary' : 'bg-gray-100'} min-h-screen transition-colors duration-200`}>
      <div className={`${isDarkMode ? 'bg-dark-secondary' : 'bg-white'} rounded-lg shadow p-6 transition-colors duration-200`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div>
            <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'} mb-1`}>Select Employee</label>
            <select
              className={`w-full pl-3 pr-10 py-2 border rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-brand-primary outline-none transition-all
                ${isDarkMode ? 'bg-dark-accent border-gray-700 text-white' : 'border-gray-300 text-gray-700'}`}
          ...
          <div>
            <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'} mb-1`}>Year</label>
            <select
              className={`w-full pl-3 pr-10 py-2 border rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-brand-primary outline-none transition-all
                ${isDarkMode ? 'bg-dark-accent border-gray-700 text-white' : 'border-gray-300 text-gray-700'}`}
              value={year}
          ...
          <div>
            <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'} mb-1`}>Month</label>
            <select
              className={`w-full pl-3 pr-10 py-2 border rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-brand-primary outline-none transition-all
                ${isDarkMode ? 'bg-dark-accent border-gray-700 text-white' : 'border-gray-300 text-gray-700'}`}
              value={month}
          ...
          <div className="flex items-end">
            <button 
              ...
              className="bg-brand-primary text-white px-4 py-2 rounded-lg hover:bg-brand-dark transition-colors flex items-center justify-center"
            >
        ...
        {reportData.length === 0 && !loading && !error && (
          <div className={`mt-6 text-center p-6 ${isDarkMode ? 'bg-dark-accent border-gray-700' : 'bg-gray-50 border-gray-200'} border rounded-lg transition-colors duration-200`}>
            ...
            <h3 className={`mt-2 text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}>No Records Found</h3>
            <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>No attendance records are available for the selected period.</p>
          </div>
        )}

        {reportData.length > 0 && (
          <div className={`${isDarkMode ? 'bg-dark-secondary' : 'bg-white'} rounded-lg shadow overflow-x-auto mt-6 transition-colors duration-200`}>
              ...
              <thead className={`${isDarkMode ? 'bg-dark-accent' : 'bg-gray-50'}`}>
                <tr>
                  ...
                  {Array.from({ length: 31 }, (_, i) => (
                    <th key={i} scope="col" className={`px-2 py-3 text-center text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>
                      {i + 1}
                    </th>
                  ))}
                  <th scope="col" className={`px-4 py-3 text-center text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>P</th>
                  <th scope="col" className={`px-4 py-3 text-center text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>A</th>
                  <th scope="col" className={`px-4 py-3 text-center text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>L</th>
                </tr>
              </thead>
              <tbody className={`${isDarkMode ? 'bg-dark-secondary divide-y divide-gray-700' : 'bg-white divide-y divide-gray-200'}`}>
                {reportData.map((employee, index) => (
                  <tr key={index} className={`${isDarkMode ? 'hover:bg-dark-accent/50' : 'hover:bg-gray-50'} transition-colors`}>
                    ...
                    <td className="px-4 py-4 text-center text-sm text-green-600 dark:text-green-400 font-semibold">
                      {employee.totals.present}
                    </td>
                    <td className="px-4 py-4 text-center text-sm text-red-600 dark:text-red-400 font-semibold">
                      {employee.totals.absent}
                    </td>
                    <td className="px-4 py-4 text-center text-sm text-yellow-600 dark:text-yellow-400 font-semibold">
