...
function Dashboard() {
  ...
  // Function to check if user should see expense data in dashboard
  const canViewExpenseInDashboard = () => {
    return hasPermission('expense:view') && userRole !== 'HR';
  };
  ...
  return (
    <div className={`container mx-auto p-4 sm:p-6 ${isDarkMode ? 'bg-dark-primary' : 'bg-gray-100'} min-h-screen transition-colors duration-200`}>
      {/* Employee Stats */}
      <div className={`grid ${userRole === 'FINANCE_MANAGER' ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'} gap-4 mb-6`}>
        {/* Total Employees Card - Visible to all roles */}
        ...
        {/* Attendance Cards - Only visible to SUPER_ADMIN, ADMIN, and HR */}
        {hasPermission('dashboard:view:attendance') && (
          <>
            <div className={`${isDarkMode ? 'bg-dark-secondary' : 'bg-white'} rounded-2xl shadow-lg p-6 transition-colors duration-200`}>
              <div className="flex items-center justify-between">
                <div>
      ...
      {/* Financial Summary Grid - Adjust grid for Finance Manager */}
      <div className={`grid grid-cols-1 ${userRole === 'FINANCE_MANAGER' ? 'md:grid-cols-2' : 'md:grid-cols-2'} gap-4 mb-6`}>
        {/* Salary Card */}
      ...
      {/* Project Overview Cards - Only visible to SUPER_ADMIN and ADMIN */}
      {hasPermission('dashboard:view:projects') && (
      ...
      {/* Project Financial Summary - Only visible to SUPER_ADMIN and ADMIN */}
      {hasPermission('dashboard:view:projects') && (
      ...
      {/* User Statistics - Only visible to SUPER_ADMIN */}
      ...
      {/* Charts Grid - Only visible to SUPER_ADMIN and ADMIN */}
      {hasPermission('dashboard:view:yearly_trends') && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {/* Yearly Salary Trend */}
          <div className={`${isDarkMode ? 'bg-dark-secondary' : 'bg-white'} rounded-2xl shadow-lg p-6 h-80 transition-colors duration-200`}>
      ...
      {/* Charts Row */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {/* Expense Categories - Only show if user can view expenses in dashboard */}
        {canViewExpenseInDashboard() && hasPermission('dashboard:view:expenses:categories') && (
          <div className={`${isDarkMode ? 'bg-dark-secondary' : 'bg-white'} rounded-2xl shadow-lg p-6 h-80 transition-colors duration-200`}>
        ...
        {/* Project Platform Distribution - Only visible to SUPER_ADMIN and ADMIN */}
        {hasPermission('dashboard:view:projects') && (
          <div className={`${isDarkMode ? 'bg-dark-secondary' : 'bg-white'} rounded-2xl shadow-lg p-6 h-80 transition-colors duration-200`}>
